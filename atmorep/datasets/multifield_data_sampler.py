...
from atmorep.utils.utils import get_weights, tokenize
...
class MultifieldDataSampler(torch.utils.data.IterableDataset):
    def __init__(
        ...
        self.range_lat += np.array([n_size[1] / 2.0, -n_size[1] / 2.0])
        # lon: no change for periodic case
        if self.ds_global < 1.0:
            self.range_lon += np.array([n_size[2] / 2.0, -n_size[2] / 2.0])
        ...
        for ifield, field_info in enumerate(fields):
            ...
            for vl in field_info[2]:
                if vl == 0:
                    ...
                    self.normalizers[ifield] += [
                        self.ds[n_name].oindex[:, :, field_idx, vl_idx]
                    ]
    ...
    def shuffle(self):
        ...
        self.idxs_perm_t = rng.permutation(self.idxs_years)[
            : self.num_samples // self.batch_size
        ]

        lats = (
            rng.random(self.num_samples) * (self.range_lat[1] - self.range_lat[0])
            + self.range_lat[0]
        )
        lons = (
            rng.random(self.num_samples) * (self.range_lon[1] - self.range_lon[0])
            + self.range_lon[0]
        )
    ...
    def __iter__(self):
        ...
        for bidx in range(iter_start, iter_end):
            ...
            for sidx in range(self.batch_size):
                ...
                lat_ran = np.where(
                    np.logical_and(
                        lats > idx[0] - ns_2[1] - res[0] / 2.0, lats < idx[0] + ns_2[1]
                    )
                )[0]
                # handle periodicity of lon
                assert not ((idx[1] - ns_2[2]) < 0.0 and (idx[1] + ns_2[2]) > 360.0)
                il, ir = (idx[1] - ns_2[2] - res[1] / 2.0, idx[1] + ns_2[2])
                if il < 0.0:
                    lon_ran = np.concatenate(
                        [np.where(lons > il + 360)[0], np.where(lons < ir)[0]], 0
                    )
                elif ir > 360.0:
                    lon_ran = np.concatenate(
                        [np.where(lons > il)[0], np.where(lons < ir - 360)[0]], 0
                    )
                else:
                ...
                sources_infos += [
                    [
                        self.ds["time"][idxs_t].astype(datetime),
                        self.lats[lat_ran],
                        self.lons[lon_ran],
                        self.res,
                    ]
                    ...
                    for ilevel, vl in enumerate(field_info[2]):
                        ...
                        if corr_type != "global":
                            ...
                            if lat_ran[0] < lat_ran[-1] and lon_ran[0] < lon_ran[-1]:
                                ...
                                normalizer = normalizer[
                                    ..., lat_ran[:, np.newaxis], lon_ran[np.newaxis, :]
                                ]
                                # cdata = normalize(cdata, normalizer, sources_infos[-1][0], year_base = self.year_base)
                        # else:
                        cdata = normalize(
                            cdata, normalizer, sources_infos[-1][0], year_base=self.year_base
                        )
                        ...
                        tok_info += [
                            [
                                [
                                    [
                                        [year, day, hour, vl, lat, lon, vl, self.res[0]]
                                        for lon in lons_sidx
                                    ]
                        ...
                        tok_info_lvl += [
                            torch.tensor(tok_info, dtype=torch.float32).flatten(1, -2)
                        ]
                ...
                for ifield, field_info in enumerate(self.fields):
                    ...
                    for ilevel, vl in enumerate(field_info[2]):
                        for ibatch in range(self.batch_size):
                            ...
                            grid = np.flip(
                                np.array(np.meshgrid(lons_idx, lats_idx)), axis=0
                            )  # flip to have lat on pos 0 and lon on pos 1
                            grid = torch.from_numpy(
                                np.array(
                                    np.broadcast_to(
                                        grid, shape=[tok_size[0] * num_tokens[0], *grid.shape]
                                    )
                                ).swapaxes(0, 1)
                            )
                            ...
                            lats_mskd_b = np.array(
                                [np.unique(t) for t in grid_lats_toked[idx_loc].numpy()]
                            )
    ...
    def set_data(self, times_pos, batch_size=None):
        ...
        for idx, item in enumerate(times_pos):
            ...
            assert item[4] >= -90.0 and item[4] <= 90.0

            tstamp = pd.to_datetime(
                f"{item[0]}-{item[1]}-{item[2]}-{item[3]}", format="%Y-%m-%d-%H"
            )

            self.idxs_perm_t += [
                np.where(self.times == tstamp)[0] + 1
            ]  # The +1 assures that tsamp is included in the range

            # work with mathematical lat coordinates from here on
            self.idxs_perm[idx] = np.array([90.0 - item[4], item[5]])
    ...
    def set_global(self, times, batch_size=None, token_overlap=[0, 0]):
        ...
        side_len_2 = side_len / 2.0
        assert all(overlap <= side_len_2), (
        ...
        for ctime in times:
            ...
            num_tiles_lat = 0
            while (lat + side_len_2[0].item()) < 180.0:
                num_tiles_lat += 1
                lon = side_len_2[1].item() - overlap[1].item() / 2.0
                num_tiles_lon = 0
                while (lon - side_len_2[1]) < 360.0:
                    times_pos += [[*ctime, -lat + 90.0, np.mod(lon, 360.0)]]
            ...
            if lat - side_len_2[0] < 180.0:
                num_tiles_lat += 1
                lat = 180.0 - side_len_2[0].item() + res[0]
                lon = side_len_2[1].item() - overlap[1].item() / 2.0
                while (lon - side_len_2[1]) < 360.0:
                    times_pos += [[*ctime, -lat + 90.0, np.mod(lon, 360.0)]]
                    lon += side_len[1].item() - overlap[1].item()
